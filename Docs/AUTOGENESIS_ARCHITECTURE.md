...
`nucleateAutogenicFounder` accepts only a local maximum with enough biomass, stored energy, catalyst, and membrane. Its inherited phase and metabolic biases come from the local spinor and chemistry rather than a predefined founder genome. `evolveAgents` integrates only force, torque, mass, and moment of inertia reduced from cells during the previous update. `separateCellularPropagules` can allocate a descendant only after a viable membrane-connected cellular component has physically disconnected from the parent. No frame, lattice tile, energy threshold, or random birth draw synthesizes an organism.
...
The owner of cell slot `i` is `i / 24`. This fixed block layout removes allocator contention and gives each cell kernel a bounded 24-cell interaction neighborhood. `evolveOrganismCells` computes symmetric same-tissue exclusion and adhesion, voltage and phase coupling, resonant strain sensing, inherited contact-propagated mechanochemical signaling, cell-local substrate gradients, ATP accounting, sparse graph dynamics, phenotype differentiation, cycle progression, and traction. `evolveCellMembranes` advances twelve explicit vertices per occupied cell and classifies exposed edges. A GPU spatial hash then resolves cross-organism membrane support overlap, localized damage, and breach-gated trophic transfer. `divideAndReduceOrganismCells` performs bounded cell division and reduces exposed-edge covariance, axes, extents, polarity, net force, torque, contact, and trophic state.

Each organism has a 96-byte developmental header, sixteen 32-byte node slots, and forty-eight 32-byte directed-edge slots. A node stores bias, response rate, sensor coupling, actuator coupling, sensor index, actuator mask, innovation ID, and activity. An edge stores weight, strain-dependent plasticity, endpoints, innovation ID, and activity. Eight inputs carry ATP, voltage, strain, contact density, morphogen polarity, local opportunity minus stress, resonant response, and membrane deformation plus junction force. Eight output channels control proliferation, adhesion, contraction, repair, permeability, secretion, apoptosis suppression, and motility. The header additionally stores heritable mechanics-to-Ca*, junction, Ca*-to-ERK*, refractory recovery, signaling-cost, traction, detachment-threshold, and propagule-investment coefficients.

Propagule separation mutates numerical parameters and can duplicate or silence nodes and add, remove, or reconnect edges. Every new structural record receives a monotonic innovation ID. The header retains active counts, topology hash, cumulative and last branch mutation distance, and structural-mutation count. Founders start from one cell. Descendants receive the exact separated cells, membrane vertices, and acquired regulatory activities together with a mutated inherited genome.
...
The reduction kernel emits a 256-byte `CellAggregate`: physiology, morphology, voltage and phase statistics, mechanics, energy terms, actuator vectors, direct equation terms, resonance, polygon geometry, mechanochemical state, exposed-edge covariance axes and extents, polarity, boundary length, net force, torque, contact load, trophic flux, and detachment readiness. Boundary covariance is the analytic line integral over exposed polygon segments, and extents project the segment endpoints rather than cell centers or a synthetic envelope. `evolveAgents` consumes the previous aggregate, while organism rendering consumes the current aggregate. This one-step delayed feedback avoids a CPU synchronization point while preserving the causal sequence:
...
Cells produced by division receive perturbed oscillator phases, frequencies, graph activities, resonator state, and polygon state. Contact phase coupling depends separately on sender and receiver phenotype, so it is generally nonreciprocal; same-tissue center-level mechanical force remains symmetric. Reproduction requires at least six parent cells, at least four cells remaining after separation, a disconnected component of one to six cells, mean propagule ATP of at least `0.48`, mean integrity of at least `0.58`, and a cell detachment score above the inherited threshold. The child receives that exact component plus mutated organism, developmental, mechanochemical, and resonance parameters.
...
6. `clearCellSpatialHash` and `buildCellSpatialHash` construct a 16,384-bucket linked-list broad phase entirely in private GPU memory.
...
8. `divideAndReduceOrganismCells` performs bounded cell division and emits exposed-edge geometry, force, torque, contact, trophic, signaling, and energetic aggregates.
9. `separateCellularPropagules` identifies disconnected viable components, transfers exact cellular state, mutates inherited parameters, and only then allocates permanent identity.
10. `evolveMechanicalField` consumes and clears contraction impulses, advances displacement and velocity, and swaps the mechanical texture pair.
11. `evolveQuantumField` advances the spinor under feedback from matter.
12. A scale-specialized full-screen pipeline renders quantum, molecular, cellular, organismal, or ecological observables without duplicating state.
13. `compactVisibleCells` rejects dead and scale-invisible cell slots, writes compacted source indices, and atomically writes an indirect instance count entirely on the GPU.
14. Cell rendering submits only compacted living twelve-triangle polygons and exposes traction, boundary exposure, contact damage, and trophic flux. Organism submission is omitted where its visibility is zero.
15. Triple-buffered observations publish stable organism positions, membrane-derived morphology, dynamics, and exact lineage transitions without stalling private state.

Periodic asynchronous reductions measure viability, activity, recovery, inherited variation, niche differentiation, trophic activity, quantum norm, cell count, division count, ATP, integrity, voltage, Ca*, ERK*, refractory state, mechanochemical edge effects, signaling cost, phase coherence, resonance, strain, membrane area, perimeter, shape index, junction force, exposed boundary, elongation, cell force, tissue torque, contact load, trophic gain/loss, detachment score, all eight inherited pathway gains, energy terms, actuator activity, graph size, and lineage mutation distance. These measurements inform the observer; they do not overwrite genomes or select a globally prescribed body.
