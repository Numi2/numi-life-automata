...
`CellState` is a 256-byte record containing organism-relative position and velocity plus fifteen state vectors:
...
Each cell has a hot 16-byte identity record containing its current physical component owner, inherited-program index, monotonic permanent cell ID, and current union-find root. Parent-cell ID remains persistent in a separate 4-byte cold genealogy buffer written only at founder creation and division. A separate 16-byte cold interaction vector stores signed ATP transfer, incompatible-program rejection, reciprocal recognition compatibility, and local net energetic contribution without expanding the 256-byte hot cell record. All 9,216 cells share one atomic allocation pool. Per-owner linked lists are rebuilt on the GPU, so tissues need not occupy contiguous records and can grow beyond 24 cells. `evolveOrganismCells` reads developmental topology, mechanochemical gains, resonant tuning, recognition coordinates, and social-control coefficients through each cell's program index. Homogeneous cells use program values cached in the component state; only cells carrying a non-dominant program read the independent program record. The kernel then computes same-tissue exclusion and adhesion, program-specific ATP sharing and rejection, voltage and phase coupling, resonant strain sensing, contact-propagated mechanochemical signaling, cell-local substrate gradients, ATP accounting, sparse graph dynamics, phenotype differentiation, cycle progression, and traction. `evolveCellMembranes` advances twelve explicit vertices per occupied cell and classifies exposed edges. A GPU spatial hash resolves cross-organism membrane support overlap, localized damage, breach-gated trophic transfer, and local connectivity candidates. `divideAndReduceOrganismCells` claims global free cells for division and traverses dynamic owner lists to reduce exposed-edge covariance, axes, extents, polarity, net force, torque, contact, trophic state, inherited-program composition, recognition, rejection, exchange, and contribution.

Each 96-byte append-only inherited program stores three trait vectors, two ligand coordinates, two receptor coordinates, fusion investment, ATP-sharing gain, incompatible-cell rejection gain, propagule-transmission gain, and genome/provenance metadata. Program storage is independent of reusable organism slots. It references a 96-byte developmental header, sixteen 32-byte node slots, forty-eight 32-byte directed-edge slots, and one 32-byte resonance record. A node stores bias, response rate, sensor coupling, actuator coupling, sensor index, actuator mask, innovation ID, and activity. An edge stores weight, strain-dependent plasticity, endpoints, innovation ID, and activity. Eight inputs carry ATP, voltage, strain, contact density, morphogen polarity, local opportunity minus stress, resonant response, and membrane deformation plus junction force. Eight output channels control proliferation, adhesion, contraction, repair, permeability, secretion, apoptosis suppression, and motility. The header additionally stores heritable mechanics-to-Ca*, junction, Ca*-to-ERK*, refractory recovery, signaling-cost, traction, detachment-threshold, and propagule-investment coefficients. Recognition and social variables are dimensionless model controls, not molecular identities or calibrated immune kinetics.

Propagule separation enumerates distinct source programs in stable cell-slot order, atomically reserves an equal-sized contiguous program range, and maps each source to an independently mutated descendant. Numerical mutation includes trait vectors, recognition coordinates, social controls, mechanochemical gains, and resonance. Structural mutation can duplicate or silence nodes and add, remove, or reconnect edges. Every new structural record receives a monotonic innovation ID. The header retains active counts, topology hash, cumulative and last branch mutation distance, and structural-mutation count. Founders start from one cell. Descendants receive the exact separated cells, membrane vertices, and acquired regulatory activities while each cell receives only the mapped descendant of its own source program. One propagule can transmit at most sixteen distinct programs; exceeding that explicit hardware bound prevents reproduction rather than collapsing program identity.
...
The reduction kernel emits a 288-byte `CellAggregate`: physiology, morphology, voltage and phase statistics, mechanics, energy terms, actuator vectors, direct equation terms, resonance, polygon geometry, mechanochemical state, exposed-edge covariance axes and extents, polarity, boundary length, net force, torque, contact load, trophic flux, detachment readiness, dominant/non-dominant program fractions, hashed program-richness lower bound, dominant-program index, mean absolute ATP exchange, rejection load, recognition compatibility, and net program contribution. Boundary covariance is the analytic line integral over exposed polygon segments, and extents project the segment endpoints rather than cell centers or a synthetic envelope. `evolveAgents` consumes the previous aggregate, while organism rendering consumes the current aggregate. This one-step delayed feedback avoids a CPU synchronization point while preserving the causal sequence:
...
4. `evolveOrganismCells` advances ATP, voltage, inherited Ca*/ERK* signaling, refractory dynamics, resonance, sparse graph state, local gradient sensing, phenotype, fate, and traction over 9,216 slots.
5. `evolveCellMembranes` advances 110,592 membrane vertices and records polygon geometry, local integrity, exposed perimeter, and detachment state.
6. `clearCellSpatialHash` and `buildCellSpatialHash` construct a 16,384-bucket spatial broad phase and dynamic per-owner cell lists entirely in private GPU memory.
7. `resolveCrossOrganismCellContacts` computes membrane support overlap, repulsion, specialized attack, local defense, breach state, and trophic transfer; `applyCellContactEffects` applies the accumulated fixed-point effects to cells and impact-facing vertices.
8. `initializeCellComponents`, `unionCellComponents`, and `compressCellComponents` label membrane-connected cells with atomic union-find, including strictly compatible cross-owner fusion; `accumulateCellComponents` measures count, centroid, ATP, integrity, and detachment readiness.
9. `selectPrimaryCellComponents` preserves continuity for the largest same-owner component, while permanent-cell ordering resolves cross-owner fusion ownership. `assignCellComponentOwners` records fusion transitions or validates each separated root, enumerates up to sixteen transmitted programs, reserves and mutates a one-to-one descendant range, and allocates permanent organism identity; `reassignCellComponents` maps every cell through its source program while transforming it into the new component frame.
10. Owner traversal lists are rebuilt after reassignment, then `divideAndReduceOrganismCells` claims global free cells for division and emits exposed-edge geometry, force, torque, contact, trophic, signaling, and energetic aggregates from those lists. The spatial contact hash remains valid only for the pre-fission contact phase and is rebuilt on the next simulation step.
11. `evolveMechanicalField` consumes and clears contraction impulses, advances displacement and velocity, and swaps the mechanical texture pair.
12. `evolveQuantumField` advances the spinor under feedback from matter.
13. A scale-specialized full-screen pipeline renders quantum, molecular, cellular, organismal, or ecological observables without duplicating state.
14. `compactVisibleCells` rejects dead and scale-invisible cell records, writes compacted source indices, and atomically writes an indirect instance count entirely on the GPU.
15. Cell rendering submits only compacted living twelve-triangle polygons and exposes traction, boundary exposure, contact damage, and trophic flux. Organism submission is omitted where its visibility is zero.
16. Triple-buffered observations publish stable organism positions, membrane-derived morphology, dynamics, birth/death transitions, and cross-owner physical-fusion transitions without stalling private state.

Periodic asynchronous reductions measure viability, activity, recovery, inherited variation, niche differentiation, trophic activity, quantum norm, cell count, division count, ATP, integrity, voltage, Ca*, ERK*, refractory state, mechanochemical edge effects, signaling cost, phase coherence, resonance, strain, membrane area, perimeter, shape index, junction force, exposed boundary, elongation, cell force, tissue torque, contact load, trophic gain/loss, detachment score, all eight inherited pathway gains, energy terms, actuator activity, graph size, lineage mutation distance, inherited-program allocation, mixed-program cell fraction, a 32-bit hashed lower bound on component program richness, mean ATP exchange, rejection load, reciprocal recognition compatibility, and net program contribution. A 384-thread gather writes the active developmental and resonance records into the metric ring instead of blitting all 4,096 program slots. These measurements inform the observer; they do not overwrite inherited programs or select a globally prescribed body.
