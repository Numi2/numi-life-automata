...
- **Direct scientific presentation.** Molecular, wave, and spinor fragments write directly to the drawable. Molecular scale adds its visible-cell molecular overlay. Wave and spinor add only compacted visible-cell body fans and measured exposed-membrane ribbons in the same render pass; they do not add junctions, program reads, a world texture, the full-resolution HDR store/reload, bloom, or a second full-screen render pass.
- **Continuous living focus.** The first founder and the morphology, cell, molecular, wave, and spinor stops use the existing permanent-birth-ID follow camera, keeping the same physical individual centered while its visual encoding changes. Manual pan and camera reset remain explicit opt-outs.
