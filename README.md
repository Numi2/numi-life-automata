...
- **Explicit residency.** Causal buffers, textures, pipelines, checkpoint banks, readback rings, and uniform arenas occupy a stable residency set. Resizable HDR and bloom targets use a two-entry, heap-backed dynamic residency pool. A cache miss never evicts a heap while an older submission can still reference it; uncommon size changes defer one render until the queue drains. Completed command contexts immediately release drawables and transient resource references. Presentation resources remain in their platform-managed residency sets.
