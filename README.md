...
| Cell state ping-pong buffers | `2 x 9,216 x 304 B` | Swift/Metal ABI-matched struct | GPU private | Metabolism, electrophysiology, mechanochemical signaling, junction-mediated morphogens, persistent developmental polarity and fate memory, ATP-funded supracellular integument and appendage state, vertex-derived exposure, traction, contact damage, trophic transfer, and environmental forcing |
...
| Cell-program interaction state | `9,216 x 16 B` | Signed ATP transfer, rejection load, recognition compatibility, net energetic contribution | GPU private | Cold structure-of-arrays output kept outside the `304 B` hot cell record |
...
| Cellular aggregates | `9,216 x 352 B` | Twenty-two `float4` vectors | GPU private | Physiology, signaling, geometry, force, trophic flux, detachment, inherited-program composition, ATP exchange, rejection, compatibility, environmental forcing, morphogen state, developmental causality, and embodied memory |
...
The next component-owner update consumes this `352 B` aggregate. With local external cell force `T_i`, recentered cell position `r_i`, tissue mass `m`, and moment of inertia `I`, motion follows `F=sum_i T_i`, `tau=sum_i r_i x T_i`, damped translation, and damped angular integration. The final vectors report inherited-program composition, measured interaction outcomes, environmental forcing, junction-coupled developmental state, and aggregated embodied memory. There is no independent cruise-speed target, pursuit impulse, separation impulse, or agent-level chemotaxis. Complete tissue loss terminates the component. This closes the causal path from local chemistry through cell signaling, membrane geometry, force, component motion, contact, feeding, fusion, and reproduction.
