...
The implementation combines eight mechanisms:
...
5. Founders start as one cell; descendants grow, differentiate, mature, repair, senesce, reproduce, and die under inherited local regulatory and mechanochemical control rather than a scripted stage machine.
...
7. Descendants inherit mutable trait vectors plus regulatory weights and topology; lifetime prediction, habituation, and acquired sensorimotor bias remain individual state and are only weakly transmissible when evolved neural tissue permits it.
8. Cells can pay energy to construct shelter, resource reservoirs, recycling chemistry, and detoxification in a seasonally disturbed world; these environmental changes create opportunities and costs without a global fitness score.
...
| Cell state ping-pong buffers | `2 x 9,216 x 384 B` | Swift/Metal ABI-matched struct | GPU private | Metabolism, electrophysiology, mechanochemical signaling, junction-mediated morphogens, persistent developmental polarity, fate memory, continuous lifecycle state, germline/soma/neural/builder allocation, acquired learning, niche work, functional surface material, traction, contact damage, trophic transfer, and environmental forcing |
| Cell occupancy | `9,216` atomic integers | `UInt32` | GPU private | Lock-free free/living/corpse state authority for one global persistent-cell pool |
| Embodied cell memories | `9,216 x 32 B` | Four packed half-precision cue/action/consequence/strength episodes | GPU private | Cell-local physical learning, decay, contact/junction replay, division inheritance, and component aggregation |
| Persistent corpse records | `9,216 x 56 B` | Atomic structure, death step, world position, donor hash, and four embodied episodes | GPU private | Occupancy-separated necromass that remains locally scavengable until depleted or decayed |
...
| Cell-program interaction state | `9,216 x 16 B` | Signed ATP transfer, rejection load, recognition compatibility, net energetic contribution | GPU private | Cold structure-of-arrays output kept outside the `384 B` hot cell record |
...
| Membrane vertices | `9,216 x 12 x 32 B` | Position, velocity, and local mechanics | GPU private | Deformable cell membranes with edge, bending, pressure, integrity, and contact state; bounded quadratic interpolation is sampled adaptively without changing simulated geometry |
| Cell spatial grid | `128²` heads + `9,216` links | Atomic `UInt32` heads and `UInt32` links | GPU private | Collision-free broad phase for living-cell mechanics, exposed-edge tests, cross-owner contact, and local corpse scavenging |
| Membrane-contact work queue | Up to `9,216 x 12 = 110,592` cell-index pairs + indirect-dispatch state | `uint2` pairs and atomic `UInt32` counters | GPU private | Each twelve-vertex membrane retains only its nearest unoccluded candidate in each support sector; this gives the pair stream a physical bound before equal-and-opposite contact mechanics and component union |
...
| Cell contact effects | Center impulses, damage, trophic exchange, reproductive material transfer, reproductive pleasure, four embodied-memory accumulators, plus `12` vertex-local force/pressure triplets per cell | Atomic fixed-point integers | GPU private | Equal-and-opposite pair accumulation and lock-free memory transmission before cell-local and membrane-local application |
...
| Cellular aggregates | `9,216 x 432 B` | Twenty-seven `float4` vectors | GPU private | Physiology, signaling, geometry, force, trophic flux, detachment, inherited-program composition, environmental forcing, developmental causality, embodied memory, lifecycle state, functional roles, acquired learning, niche work, and surface material |
| Developmental headers | `4,096 x 256 B` | Topology, mutation distances, actuator biases, mechanochemical coefficients, morphogen kinetics, junction material, ecological response, lifecycle, neural-plasticity, niche-construction, and morphogenesis/canalization parameters | GPU private | Program-indexed bounded sparse graph metadata plus inherited mechanochemical, material, ecological, life-history, learning-rule, and body-plan parameters |
...
| Registered junction indices | `32,768 x 4 B` | Append-once junction-slot indices | GPU private | Exact persistent-junction render candidates without rescanning empty hash slots; stale or reused slots remain validated from causal state |
...
| Junction indirect arguments | `3 x (4 + 3) x 4 B` | Draw and compute-dispatch arguments | GPU private | Slot-local GPU-rebuilt and bounded registered-candidate dispatch plus visible-junction draw counts |
...
| Metric readback ring | `3` slots, including `9,216 x 224 B` component-program records per slot | Structured buffers | Shared | Asynchronous population reductions, cellular aggregates, functional morphology, and GPU-gathered active-program measurements |
...
From `64x` through `160x`, the renderer follows the physical cells rather than displaying the extracellular reaction lattice. Each visible cell retains its simulated membrane contour and supracellular integument. Exposed arcs extend the maintained collective surface, internal membranes become subordinate to the continuous tissue, and only the physical outer contour survives into wave scale, so a multicellular component remains visibly one organism while its intracellular state is exposed.
...
Each `CellState` is a `384 B` GPU record:
...
lifecycle = (ontogenetic progress, reproductive maturity,
             senescent load, regenerative reopening)
roles = (germline-like, soma-like, neural-like, niche-builder allocation)
learning = (cue prediction, prediction error, habituation, acquired response)
niche = (paid shelter, reservoir, recycling, detoxification activity)
```
...
Each live program references a separate `256 B` developmental header, sixteen `32 B` node slots, forty-eight `32 B` directed-edge slots, and one `32 B` resonance record. Founders begin with twelve active sensor-coupled nodes and twenty active edges. The header carries heritable gains for mechanics-to-Ca*, junction transmission, Ca*-to-ERK*, refractory recovery, signaling ATP cost, traction, detachment threshold, propagule investment, junction adhesion, cortical tension, viscous damping, junction permeability, toxin tolerance, detrital scavenging, shear anchoring, starvation quiescence, maturation, senescence, regeneration, germline and soma allocation, neural plasticity, cultural transmission, niche construction, organizer persistence, segmentation frequency, bilateral coupling, outgrowth, developmental plasticity, canalization, material retention, and injury-driven reopening. It also stores independent basal production, first-order decay, receptor sensitivity, and junction diffusivity coefficients for two dimensionless morphogens. A node stores its bias, response rate, sensor weight, actuator weight, sensor index, twelve-bit actuator mask, activity flag plus developmental-module ID, and monotonic innovation ID. An edge stores its weight, strain-dependent plastic term, source, target, activity flag plus module ID, and monotonic innovation ID. Inactive slots have zero effect but can be activated by structural mutation.

The twenty-four cell-local inputs are:
...
q = (ATP, membrane voltage, extracellular strain, contact density,
     ...
     reproductive maturity, senescent load, prediction error,
     locally constructed niche support)
```
...
Multicellular bodies can construct a supracellular integument from their actual exposed membrane arcs. Adhesive, repair, and secretion activity build coverage only while ATP and membrane integrity support the work; wounds, toxin, deformation, and starvation erode it. Its tension propagates through persistent junction conductance, strengthens junction material, reduces local breach probability and environmental stress, retains a bounded share of substrate access, and improves paid traction. Motile lateral boundary cells can further specialize the same physical membrane into small bilateral feet: a narrow leg and distal pad alternate stance by half a gait cycle, amplify resource-directed traction, and resist terrain slip through matrix and shear anchoring. Construction, maintenance, and every gait cycle consume ATP. The current multicellular flag gates both geometry and force, so a lone cell cannot retain the locomotor advantage. Cytokinesis partitions the constructed material, so neither a sheath nor a foot is copied for free. No observer label grants these benefits.
...
Physical component separation neither mutates nor remaps a cell's program. Every disconnected nonempty root receives a component handle, and every cell retains its existing generation-tagged program, acquired regulatory state, membrane geometry, and permanent identity. Mixed-program propagules therefore transmit their actual cellular composition. A fragment reopens its developmental clock in proportion to the germline-like state physically carried by its cells; soma-heavy fragments retain most of their age, while sexual offspring receive an additional bounded reset. Physical descent is recorded immediately, but regenerative reproduction is not: a separated descendant must restart cell division, receive a scheduled wound on at least one actual cell, and then sustain a multicellular, ATP-supported, intact, low-stress state through the recovery window. These milestones are historical observation flags and never gate component allocation or survival.

### Emergent life histories, learning, and constructed niches

There is no `juvenile`, `adult`, or `dead` mode switch in the causal simulation. Every living cell continuously integrates ontogenetic progress, reproductive maturity, senescent load, and injury-driven developmental reopening. The rates and thresholds are inherited and mutable. Energy, integrity, stress, local signals, cell role, and tissue geometry alter the trajectory, so metamorphic transitions, prolonged juvenility, disposable soma, germline-like propagules, regenerative adulthood, and weak or absent senescence are possible outcomes of the same equations. Maturity only permits reproductive investment; a real descendant still requires ATP-funded division, physical detachment, and continued survival. Senescence raises maintenance and membrane wear. Terminal cell loss returns accounted material to detritus and heat, while persistent corpses can be scavenged before their slots are recycled.

The same local regulation continuously allocates germline-like, soma-like, neural-like, and niche-builder functions. These are overlapping numerical roles, not named cell classes. Germline allocation increases propagule investment and rejuvenation; soma allocation supports maintenance and repair but can retain age; neural allocation enables prediction, habituation, and an acquired sensorimotor bias; builder allocation enables environmental construction. Division attenuates acquired state to `8-38%` and retained episodes to `16-58%`. Contact can transmit a still smaller episode only when both organisms express neural tissue and inherited cultural-transmission gain. The genome therefore carries developmental and learning rules, while experience remains principally an individual property.

Builder-like cells pay active-work energy for four external effects. Shelter and reservoirs deposit persistent matrix that changes shielding, retention, and local resource capacity. Recycling secretes catalyst into the existing detritus-to-resource pathway, allowing one lineage's remains to support another. Detoxification neutralizes local toxin. These benefits are spatial, finite, and available to neighbors, so exploitation, cooperation, succession, and niche inheritance can arise without assigning ecological roles. Spatially offset short and long seasons alternately favor resource A and B and introduce bounded disturbance/toxin pulses. Predation, recognition, fusion, rejection, corpse scavenging, matrix construction, and cross-program exchange remain local physical interactions; no scalar fitness or organism-level niche label is consulted.
...
Terminal loss also changes the slot from living occupancy `1` to corpse occupancy `2`. The corpse retains bounded physical structure, world position, donor provenance, and its four retained embodied episodes; it cannot enter living compaction, topology, invariants, or rendering and cannot be reused while structure remains. Nearby scavengers atomically deplete structure while drawing energy only from the already-accounted local detritus field, so necromass cannot duplicate energy. Contact can transmit a weakened donor episode, letting a scavenger reenact acquired behavior without copying a stat or bypassing selection. Depletion or bounded decay returns the slot to free occupancy `0` inside the existing spatial-grid pass.

Each living cell keeps four packed half-precision episodes `(cue, action, consequence, strength)`. The cue compresses local substrate, toxin, catalyst, matrix, wound, strain, and frequency mismatch; the action compresses expressed contraction, motility, secretion, and permeability; consequence is measured from ATP, integrity, stress relief, and resource capture. Repeated outcomes strengthen a signed episode while obsolete evidence decays. Replay perturbs the existing resonator, Ca*, ERK*, voltage, and paid-traction equations. Existing contact and junction resolution transmits bounded episodes, the component reduction aggregates them, and division inherits attenuated parental memory with a bounded second-parent contribution after recombination. Matrix and catalyst therefore remain external constructed memory. This adds two compact per-cell buffers but no CPU controller, world texture, or simulation pass.
...
Four additional graph outputs express local construction demand. Body-relative axial and lateral position, inherited segmentation phase, morphogens, voltage, resonance, junction strain, polarity, growth, and injury determine where that demand occurs. ATP then converts it gradually into persistent predatory, armor, sensory, or locomotor surface material. Stable closure suppresses remodeling; inherited retention preserves useful construction; starvation, damage, and disuse erode it; injury locally reopens plastic development. Armor raises local stiffness and defense while reducing uptake and motion. Predatory material extends contacted membrane and enables breach. Sensory material amplifies existing morphogen, resonance, and resource-gradient responses while remaining exposed. Locomotor material constructs and actuates terrain-coupled appendages. Cytokinesis partitions all four materials between daughters instead of copying a completed structure.
...
The next component-owner update consumes this `432 B` aggregate. With local external cell force `T_i`, recentered cell position `r_i`, tissue mass `m`, and moment of inertia `I`, motion follows `F=sum_i T_i`, `tau=sum_i r_i x T_i`, damped translation, and damped angular integration. The final vectors report inherited-program composition, measured interaction outcomes, environmental forcing, junction-coupled developmental state, aggregated embodied memory, lifecycle state, roles, acquired learning, niche work, and persistent functional surface material. There is no independent cruise-speed target, pursuit impulse, separation impulse, or agent-level chemotaxis. Complete tissue loss terminates the component. This closes the causal path from local chemistry through cell signaling, membrane geometry, force, component motion, contact, feeding, fusion, reproduction, aging, death, and recycling.
...
The application requires macOS 26 and uses a Metal 4-only submission backend. `Metal4ExecutionContext` owns one `MTL4CommandQueue`, three reusable submission slots, one `MTL4CommandAllocator` per slot, GPU-addressed uniform arenas, a stable residency set, per-submission render-target residency sets, and commit-feedback state. Each slot also owns its visible-index lists, indirect draw arguments, and resizable full-resolution HDR/bloom heap. Admission is nonblocking and the interactive renderer permits at most two unfinished display submissions. An allocator or render working set is reused only after its feedback handler has reported completion. Simulation work is segmented at the existing x24 interaction boundary, repeated world expansions are spread across submissions, and each step is admitted only when the uniform arena still has a conservative render-tail reserve. These scheduling bounds prevent allocator overruns; scale-specific direct presentation and compact tissue shading reduce work without changing simulation resolution or causal state.
...
Release builds load `Replicator.metallib`, compiled with `-std=metal4.0`, and create pipelines through `MTL4Compiler`; they never compile MSL source. `Replicator.mtl4archive` remains packaged for qualification, but runtime archive lookup is disabled by default after the macOS 26 M4 driver produced both an archive-fallback exception and perspective-dependent raster corruption. `NUMI_ENABLE_METAL4_ARCHIVE=1` is reserved for the device-specific corruption soak. Full release packaging still fails when either AOT artifact is absent so archive qualification cannot silently drift from the metallib. For a local machine on which Apple's offline archive compiler is unavailable or wedged, `./Scripts/install-macos-app.sh --runtime-only` rebuilds and packages the metallib used by normal execution and explicitly omits the optional archive; that install is not evidence of archive qualification.

Every `1,200` scientifically committed steps, the renderer alternates between two complete in-memory causal checkpoints. If a commit reports an error or the five-second completion watchdog observes stalled submissions, new submission stops and the epoch advances. A drawable is presented only from successful Metal feedback for an epoch that is still current; failed, overdue, or late-retiring work leaves the last accepted image on screen. The renderer waits for real Metal feedback to retire every old command buffer before reusing any causal or render resource; it then rebuilds the queue, allocators, uniform arenas, timestamp heaps, residency bindings, and argument tables and restores the last complete checkpoint with matching CPU observer state. Late callbacks from the retired epoch cannot present, commit observations, or update counters, and recovery never initializes a replacement world.
...
| Direct scientific display | One direct-to-drawable full-screen triangle; molecular cells at `64-160x`, then a faint physical body fill and measured outer membranes from `160x` through spinor inspection | `molecularDisplayFragment`, `waveDisplayFragment`, or `spinorDisplayFragment` |
...
- **Native Metal 4 submission.** Three reusable allocator slots encode through `MTL4CommandBuffer`, submit through `MTL4CommandQueue`, and retire through commit feedback. The CPU never blocks to admit a fourth unfinished submission, and only a successfully retired submission may present its drawable.
- **Submission-local argument tables.** Each of the three allocator slots initially owns one compute table and six render-stage table pairs. A slot resets its table cursor only after commit feedback retires the preceding submission, eliminating per-frame table allocation while preventing in-flight mutation across slots. Capacity denial fails the speculative submission closed, grows the recovered slot capacity, and replays from preserved state instead of indexing beyond the table arrays.
- **Capacity-aware work segmentation.** The x24 setting remains x24, but its steps are admitted individually against a conservative uniform-arena reserve and expansion requests are distributed across submissions. Uniform writes are checked, never asserted into memory; an unexpected exhaustion fails the speculative submission, grows the recovered arenas, and replays the preserved state. Normal workload growth becomes another frame or a larger arena rather than an allocation overrun or a rendering-quality reduction.
...
- **Physically bounded contact work queue.** One spatial-hash traversal retains the nearest unoccluded candidate for each of a cell's twelve real membrane-support sectors. It can therefore emit at most `live cells x 12` pairs by construction, with no arbitrary linked-list cutoff or population-quadratic queue capacity. Contact mechanics and connectivity consume that same indirect pair stream, avoiding a second neighborhood search while retaining equal-and-opposite fixed-point force accumulation.
- **Causality-aware multirate development.** Mechanics, contact, energy, death, division, signaling, and transport remain exact every biological step. Wounded, dividing, strained, feeding, reproducing, or rapidly signaling cells also update their regulatory graph every step. Stable exposed cells update every second step and stable interior cells every fourth, staggered by permanent cell ID. On an update, `1-(1-r)^n` analytically preserves each node's inherited relaxation timescale across `n` held-input steps; skipped cells retain their last causal actuator state.
- **GPU-deadline biological pacing.** Interactive command-buffer completions estimate render-only cost and simulation-step cost with separate exponential moving averages, periodically calibrated by precise Metal counter timestamps for the scientific display, scene raster, bloom, and composite phases. A hysteretic `12.5 ms` controller immediately reduces biological steps per submitted frame when the measured workload grows and recovers them slowly when headroom returns. It never changes cell capacity, culls a live organism from simulation, lowers membrane geometry, disables scientific layers, or changes the fixed-step headless workflow.
- **Component-level topology invalidation.** Birth, death, fusion, a new bridge between different components, or complete local isolation triggers union-find reconstruction immediately. Mature junctions use separate make/break distances, so ordinary membrane remodeling does not masquerade as organism fission. A periodic 256-step exact reconstruction catches nonlocal graph cuts without rebuilding a stable body for harmless edge churn. Initialization clears only living roots, so stable tissues do not repeatedly clear and scan all `9,216` component slots.
...
- **Static deep-scale fragments.** Molecule, wave, and spinor views use independent MSL 4.0 entry points. Molecular scale has a constant-cost dark background plus compacted physical cells; it samples no world or quantum textures. Wave rendering uses one quantum sample and one prepared-coupling sample. Wave and spinor inspection then reuse one compacted visible-cell list for a faint physical body interior and the measured twelve-edge exposed membrane, with no junction or heritable-program reads.
...
- **Morphology from membrane construction.** The renderer contains no abdomen, jaw, spine, organism sprite, decorative limb primitive, or separate organism mesh. A continuous organism silhouette appears only when exposed cells have paid to construct integument and transmit its tension through physical junctions. The exposure bit already stored on each membrane edge expands only the real outer arcs in the existing cell-contour vertices; tension fades internal membranes, and junction material is drawn underneath as connective tissue. At ecological overview scale, those same causal junctions remain visible as simplified connective tissue, multicellular contours receive a bounded sub-pixel footprint, and solitary cells recede; the overview therefore preserves whole individuals instead of showing disconnected dots. The overview treatment blends into detailed morphology from `6-9x`. Molecular inspection retains cellular contents and the collective outer surface; wave and spinor inspection retain a faint body interior plus the same measured outer membrane, so crossing a scientific scale boundary no longer drops physical organism context. This costs no new compaction buffer, geometry buffer, render pass, world texture, or full-screen sample. Multicellular-only feet are narrow and dark with a stance-lit sole, making a moving individual distinct from a free cell. Their leg and distal pad are elongated simulated membrane vertices, and the displayed alternating contact phase is the same state that produces paid terrain traction; only their existing direction receives a bounded minimum projection when the physical pad would otherwise fall below a pixel.
- **M4 adaptive mesh contours.** The M4 profile dispatches one Metal 4 mesh threadgroup per visible cell and emits eighteen segments below `6x` or thirty-six segments at cellular inspection scales. The generic Metal 4 profile retains the indexed vertex path; both interpolate the same twelve physical membrane vertices with derivative antialiasing.
...
- **Direct scientific presentation.** Molecular, wave, and spinor fragments write directly to the drawable. Molecular scale adds its visible-cell molecular overlay. Wave and followed-organism spinor views add only compacted visible-cell body fans and measured exposed-membrane ribbons in the same render pass. The unfocused launch spinor instrument skips that unused biological compaction and overlay work. None adds junctions, program reads, a world texture, the full-resolution HDR store/reload, bloom, or a second full-screen render pass.
- **Intentional living focus.** A fresh launch stays centered on the initialized quantum cell so both spinor components are visible immediately. Entering a biological observation stop or using the follow controls opts into the permanent-birth-ID camera, keeping the same physical individual centered while its visual encoding changes. Manual pan and camera reset remain explicit opt-outs.
...
- **GPU-compacted junction submission.** Every junction-table slot enters an append-once GPU registry the first time it is claimed. `compactVisibleJunctions` indirectly dispatches only over that high-water registry, then validates pair fingerprints, occupancy, owner continuity, stale age, finite material state, tracking, and viewport bounds. Each allocator slot owns its writable dispatch arguments, visible index list, and draw arguments. One indirect instanced draw overlays only surviving physical junctions, with no CPU traversal, readback, or unconditional `32,768`-slot render scan.
- **Scale-adaptive cell boundaries.** The indirect draw expands each compacted cell index into an eighteen-triangle quadratic fan through dense tissue and a thirty-six-triangle fan only at molecular inspection. Both sample the same twelve simulated membrane vertices.
- **Compact causal cell shading.** Above `18x`, a coherent fast path retains lineage, role, ATP, voltage, membrane integrity, paid repair, pressure, strain, integument, Ca*, ERK*, traction, trophic exchange, stress, apoptosis, and the scientific display modes while omitting sub-pixel organelles and traveling rings. Closer views retain the richer physical-cell treatment.
- **Four-sample dense context.** The cellular surface reads state, ecology, geology, and mechanics once per pixel. Quad derivatives are scaled back to one simulation texel to preserve the biomass- and displacement-gradient diagnostics without four explicit neighbor samples; wave direction comes directly from measured mechanical velocity.
- **Scale and viewport cull before submission.** The same compacted living-cell list remains available above `0.35x`; ecology through molecular scales render the selected cell detail, while wave and spinor scales use only the minimal body-context and exposed-membrane vertices. Junction ribbons stop at `64x`. A bounded world-space viewport test remains finite at extreme magnification and prevents off-screen membrane fans from entering vertex binning or fragment shading.
...
For the body-plan scaling change, an interleaved controlled run rebuilt exact commit `aacfaff` and the current source on the same M4, then ran both with `--steps 12000 --seed 8 --batch 32 --sample-every 1200 --audit-every 1200 --quantum-stride 3`. Under the same sustained desktop GPU load, the baseline completed at `1,293.7 steps/s`; the bounded-sector, multirate version completed at `1,240.5 steps/s`, or `95.9%` of baseline. The current trajectory performed one fission and one fusion, ended with four living cells in one component, retained exact zero energy residual, and reported invariant flags `0x0`. This controlled ratio is the acceptance measurement; absolute rates vary with concurrent WindowServer and Metal load.
...
Interactive execution uses one command queue with three allocator slots and at most two unfinished display submissions. Admission is nonblocking. The interface reports scheduled, GPU-completed, and scientifically committed steps separately, plus measured steps per second, queue depth, checkpoint step, and recovery count. Every completion status and error is inspected with encoder execution diagnostics enabled. Drawable presentation is gated by successful completion feedback and current-epoch admission, so partial, failed, or late-retiring raster output cannot replace the last valid image. A five-second completed-frame watchdog stops further submission, invalidates presentation for the overdue epoch, quarantines its resources until Metal actually retires them, and only then restores the preserved world.
...
For each fixed SplitMix64-derived seed, the runner executes the same paired sham and targeted-wound branches. A valid cycle requires an exactly matched eligible baseline target, a delivered wound, and clean invariants in both branches. The wounded descendant must first recover relative to its sham twin. It must then undergo a physical fission, and that direct child must cross the live reproductive-maturity threshold and remain multicellular and metabolically viable across the declared maturation window after restarting its own cell division. This is the observed parent-tissue → independent descendant → recovery → descendant fission → mature viable grandchild chain.
...
10. **Reduced biomechanics.** Polygon forces are dimensionless edge, bending, area, contraction, membrane-support contact, supracellular-integument, appendage, and persistent-junction terms; the extracellular medium combines a bounded damped vector wave with one scalar matrix-density field. Neither is a calibrated viscoelastic constitutive model, finite-element tissue, measured extracellular matrix chemistry, or resolved epithelial sheet. Cross-lineage fusion is a thresholded adhesion rule, not a measured fusion pathway. The collective boundary is assembled from exposed cell arcs rather than a separate free-standing organism mesh.
